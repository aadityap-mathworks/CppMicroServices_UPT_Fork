...
class BundleInstallFixture : public ::benchmark::Fixture
{
  public:
    ...
    static std::vector<cppmicroservices::Bundle>
    ConcurrentInstallHelper(cppmicroservices::Framework& framework, std::vector<std::string> const& bundlesToInstall)
    {
  ...
  protected:
    ...
    void
    InstallConcurrently(benchmark::State& state, uint32_t numThreads)
    {
        using namespace std::chrono;
        ...
        std::transform(str5kBundles.begin(),
                       str5kBundles.end(),
                       str5kBundles.begin(),
                       [&count](std::string& s) -> std::string { return s.append(std::to_string(count++)); });
        ...
        {
            bundlesToInstallPerThread.push_back(std::vector<std::string>(lowerBound, upperBound));
        ...
        for (auto _ : state)
        {
            ...
            for (auto const& bundlesToInstall : bundlesToInstallPerThread)
            {
...
BENCHMARK_REGISTER_F(BundleInstallFixture, ConcurrentBundleInstall1ThreadPerBundle)->UseManualTime();
#endif
